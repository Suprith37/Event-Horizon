...
var sql23 = `
SELECT et.Event_name as Event_name , r.Email as Email from event_registration et join Sign_up r on et.User_id = r.Id 
  ...
  con.query(sql23 , ( err, result)=>{
    ...
    result.forEach(event=>{
      const sq56= `
        ...
        WHERE 
          er.event_name LIKE '${event.Event_name}' AND er.End_date < CURDATE() OR ( er.End_date = CURDATE() OR ( er.End_time < CURTIME()));
      `;
  ...
  //   if(err) console.log(err);
    ...
    con.query(sql,(err,result)=>{
     ...
     if(result[i].Email === email && result[i].Password === pass){
      ...
      current_user_id = result[i].Id;
      curr_user_name = result[i].First_name;
...
// -------------------  ^^^^^^||||||^^^^^^  --------------------------
      ...
      // req.session.saveUninitialized = true;
        ...
        console.log("Successful login "+current_user_id+" name: "+result[i].First_name);
        current_user= true;
      ...
      console.log(current_user," ",current_user_id);
        ...
        // res.redirect("/user");
        var Name = result[i].First_name.substring(0,1)+ result[i].First_name.substring(1,result[i].First_name.length).toLowerCase();
        console.log(Name);
...
// else{
//   var sql = `SELECT Id , Event_name, Event_type , Hour(Start_time) as hour, MINUTE(Start_time) as min ,Day(Start_date) as day,MONTHNAME(Start_date) m_name ,   College_name, About_event, image_path from event_registration `;
//   con.query(sql,(err,result)=>{
...
where r.User_id = ${req.session.user.user_id} AND e.Start_date > CURDATE() OR (Start_date = CURDATE() AND e.Start_time > CURTIME())

),
LiveEvents AS (
 SELECT er.Id, Event_name, Event_type, HOUR(End_time) as E_hour, MINUTE(End_time) as E_min, DAY(End_date) as E_day, MONTHNAME(End_date) as E_m_name,Year(End_Date) as E_year , HOUR(Start_time) as hour, MINUTE(Start_time) as min, DAY(Start_date) as day, MONTHNAME(Start_date) as m_name, College_name, About_event, image_path,
   'Live' AS event_status , (Select Count(*) from Registration_table r where r.Event_id = er.Id) As total FROM event_registration er 
WHERE Start_date <= CURDATE() AND (Start_date < CURDATE() OR (Start_date = CURDATE() AND Start_time <= CURTIME())) AND End_date >= CURDATE() AND (End_date > CURDATE() OR (End_date = CURDATE() AND End_time >= CURTIME())) 
...
// --------------------------------  search events  ---------------------------------
    ...
    var sql = ` SELECT Id, Event_name, Event_type, HOUR(End_time) as E_hour, MINUTE(End_time) as E_min, DAY(End_date) as E_day, MONTHNAME(End_date) as E_m_name,Year(End_Date) as E_year , HOUR(Start_time) as hour, MINUTE(Start_time) as min, DAY(Start_date) as day, MONTHNAME(Start_date) as m_name, College_name, About_event, image_path
    , (select count(*) from Registration_table rr where rr.Event_id = e.Id ) as total ,'Live' AS event_status FROM event_registration WHERE Start_date <= CURDATE() AND (Start_date < CURDATE() OR (Start_date = CURDATE() AND Start_time <= CURTIME())) AND End_date >= CURDATE() AND (End_date > CURDATE() OR (End_date = CURDATE() AND End_time >= CURTIME()))  AND College_name Like "%${req.query.q}%" Group by Id;`
...
con.query(sq13,[event_id, req.session.user.user_id],(err,result)=>{
  if(err) console.log(err);
